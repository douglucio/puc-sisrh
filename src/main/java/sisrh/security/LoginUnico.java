...
public class LoginUnico {
	...
	public static Usuario buscarUsuario(String usuario, String senha) throws Exception {
		try {
			...
			String sql = "SELECT nome, senha, perfil, matricula " + "FROM Usuario " + "WHERE nome = ? and senha = ?";
			...
			while (rs.next()) {
				Usuario us = new Usuario(rs.getString("nome"), rs.getInt("perfil"), rs.getString("matricula"), null);
				return us;
	...
	public static String geraToken(String usuario, String senha) throws Exception {
		...
		if (user != null) {
			...
			String jwtToken = Jwts.builder().setHeader(headers).setIssuedAt(new Date()).setClaims(claims)
					.setSubject("Acesso SISRH").setIssuedAt(dtCriacao).setExpiration(dtExpiracao)
					.signWith(LoginUnico.getPrivateKey()).compact();
			return jwtToken;
		}
		return null;
	}
	...
	public static Jws<Claims> validarToken(String tokenJWT) throws Exception {
		try {
			Jws<Claims> credencial = Jwts.parserBuilder().setSigningKey(getPrivateKey()).build()

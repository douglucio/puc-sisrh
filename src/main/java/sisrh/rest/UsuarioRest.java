...
public class UsuarioRest {
	...
	public Response listarUsurios() throws Exception {
	...
	@Path("{matricula}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response obterUsuarioPorMatricula(@PathParam("matricula") String matricula) throws Exception {
		try {
			Usuario usuario = Banco.buscarUsuarioPorMatricula(matricula);
			if (usuario != null) {
				return Response.ok().entity(usuario).build();
			} else {
				return Response.status(Status.NOT_FOUND).entity("{ \"mensagem\" : \"Usuario nao encontrado!\" }")
						.build();
			}
		} catch (Exception e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR).entity(
					"{ \"mensagem\" : \"Falha para obter usuario!\" , \"detalhe\" :  \"" + e.getMessage() + "\"  }")
					.build();
		}
	}
	...
	public Response incluirUsuario(Usuario usuario) {
		try {
			Usuario user = Banco.incluirUsuario(usuario);
	...
	public Response alterarUsuario(@PathParam("matricula") String matricula, Usuario usuario)  {
		try {			
			...
			Usuario user = Banco.alterarUsuario(matricula, usuario);	
			return Response.ok().entity(user).build();
		}catch (Exception e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR)
